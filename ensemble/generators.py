...
def place_in_register(pitch_class: int, register: Tuple[int, int]) -> int:
    """Lowest MIDI note >= register[0] with this pitch class. Shared with
    ensemble/comping.py, which is why this isn't a private (leading-underscore)
    helper any more."""
    low, high = register
...
def chord_tone_generator(register: Tuple[int, int]) -> Generator:
    ...
    def generate(song, bar_index: int, timeline, director_signal) -> List[NoteEvent]:
        ...
        for beat_offset in (0.0, 2.0):
            ...
            fifth = place_in_register(fifth_class, register)
