...
Ported from wolfson's data/scales.py — see ensemble/wolfson/__init__.py for
what changed mechanically vs. the source. Only chord_tones() is actually
load-bearing for combo's use (phrase_generator.py's voice-leading bias calls
it unconditionally) — the rest of this module's functions ship along with it
unused, same reasoning as chords.py's unused parse_chord.
"""

from .chords import QUAL_MAJOR, QUAL_DOM, QUAL_MINOR, QUAL_DIM
...
def chord_to_mode(chord_idx: int) -> str:
    ...
    chord_idx: combined index from chords.py (root * N_QUALITIES + quality)
    """
    from .chords import NC_INDEX, N_QUALITIES
...
def tritone_sub(chord_idx: int) -> int:
    ...
    """
    from .chords import NC_INDEX, N_QUALITIES, QUAL_DOM
    if chord_idx == NC_INDEX:
        return chord_idx
    root    = chord_idx // N_QUALITIES
    quality = chord_idx % N_QUALITIES
