...
Vocabulary (output tokens) — defined in encoding.py:
...
Chord conditioning — defined in chords.py:
...
Ported from wolfson's generator/lstm_model.py — see ensemble/wolfson/__init__.py
for what changed mechanically vs. the source. LSTM_HIDDEN_SIZE and
LSTM_NUM_LAYERS are inlined (not imported from a config module) because they're
architecture-defining and must exactly match the shapes baked into the trained
sax_best.pt checkpoint.
"""

import torch
import torch.nn as nn

from .encoding import VOCAB_SIZE, N_PITCHES, N_DUR_BUCKETS, DUR_OFFSET, END_TOKEN
from .chords import CHORD_VOCAB_SIZE, NC_INDEX
