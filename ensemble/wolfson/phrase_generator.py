...
Ported from wolfson's generator/phrase_generator.py — see
ensemble/wolfson/__init__.py for what changed mechanically vs. the source
(relative imports in place of sys.path + absolute imports; four constants
from wolfson's own config.py — MAX_GENERATED_NOTES, GENERATION_TEMPERATURE,
DEFAULT_INSTRUMENT, REST_PITCH — inlined below instead of imported). No
algorithmic content in this file has been changed.

combo does not port wolfson's ArcController/HarmonyController, which supplied
this module's ~12 bias-layer parameters (contour_target, phrase_energy_arc,
motif_targets, modal_strength, register_contrast_str, etc.) with musically
considered values during live performance. ensemble/sax.py calls generate()
with their defaults instead — DESIGN.md §12 records this as a deliberate,
...
from .chords import NC_INDEX, CHORD_VOCAB_SIZE
from .scales import chord_tones
from .lstm_model import PhraseModel

# Inlined from wolfson's config.py — see module docstring above.
...
class PhraseGenerator:
    ...
    def generate(
        ...
        max_phrase_beats    hard beat-accumulator ceiling.  Defaults to the global
                            ...
                            NOTE (combo integration): this does NOT bound the
                            returned phrase's total span — rest injection (see
                            _inject_rests below) runs after the beat-accumulator
                            check and adds unaccounted-for silence. Callers that
                            need a hard span guarantee (e.g. combo's per-bar
                            clipping in ensemble/sax.py) must clip the returned
                            notes themselves; verified empirically, not assumed.
...
def _apply_pitch_range_bias() -> torch.Tensor:
    from .encoding import PITCH_MIN
